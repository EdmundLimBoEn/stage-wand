...
Stage Wand turns an iPhone into a pocket presentation clicker and a trackpad for your Mac. Volume up sends Right (next slide); volume down sends Left (previous slide) to the frontmost Mac app. Unlock the phone and enable ARM to move the pointer, click, scroll, and switch spaces.
...
This runs `make app` in `mac/`, builds the release executable, and wraps it in a menu bar app. Open its menu bar popover to see the pairing code, connection status, and listening port. The default port is **8787**, with fallback ports **8788–8790**; always use the port shown there.
...
5. Open Stage Wand's phone Settings and type the four-digit pairing code shown in the Mac popover, including any leading zeros. Wait for the authenticated/connected state before testing controls.
...
**ARM gates pointer movement, clicks, scrolling, and three-finger gestures. Keys, including volume presses, work with ARM off.** ARM resets when the phone app enters the background; enable it again after unlocking for pointer control. Adjust sensitivity in phone Settings (0.5–3).
...
| Connected but the Mac ignores input | Re-grant Accessibility after a rebuild, enable ARM for pointer controls, and put the intended Mac app frontmost. |
...
| Phone was locked or connection dropped | Unlock and reopen Stage Wand; it reconnects. Wait for authentication, then re-enable ARM. Up to eight queued key/click/chord commands may arrive after reconnecting. |
...
| Three-finger swipes do nothing | Enable ARM and the Mission Control shortcuts, and create another desktop for left/right switching. |
